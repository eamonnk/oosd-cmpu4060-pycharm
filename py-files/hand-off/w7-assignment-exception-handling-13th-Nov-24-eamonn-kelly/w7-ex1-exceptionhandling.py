'''
Week 7 Assignment - Exception handling-Ex 1
...
# conditions for len and valid numbers:
    if sorted(valid_list_range_str) != sorted(frnd_pref) or len(valid_list_range_str) != len(frnd_pref):
        print("Friend - You have entered values not in range or too many numbers. You must enter numbers between 1 and {}. ".format(REQ_LIST_LEN))
        num_tries_2 += 1
        continue
    else:
        print("You have entered valid preferences")
...
try:
# two lists one sorted based on order of the other. Using lambda function, specifyin int as value is a str in list
