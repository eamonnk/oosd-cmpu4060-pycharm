...
def add_item():
    ...
    add_quantity=int(input("Number in Stock: "))
...
def update_item():
    ...
    groceries[update_item_ID][3]= int(input("Please enter updated Stock Number: "))
    print_all()
...
def update_stock():
    ...
    for items in range(len(groceries)):
        if groceries[items][-1] <=2:
            ...
            pass
    if groc_stock:
        print("The following items have low stock. ")
        for items in range(len(groc_stock)):
            print("***  "+ str(groc_stock[items][0])+"  ***")
    else:
        print("There are no items with low stock")
    input("Press Enter to continue and display latest stock information: ")
    # msvcrt.getch()
